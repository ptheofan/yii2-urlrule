...
class UrlRule extends Object implements UrlRuleInterface
{
    ...
    /**
     * Return the value of a param. Used when param value is object
     * @param $param
     * @return string
     */
     ...
     * @param string $param
     * @param mixed $value
     * @return bool false if route should not match due to invalid parameter
     */
    ...
    public function createUrl($manager, $route, $params)
    {
        ...
        foreach ($this->_paramRules as $name => $rule) {
            if (isset($params[$name]))
                $value = $this->getParamValue($params[$name]);

            if (isset($value) && !is_array($value) && ($rule === '' || preg_match($rule, $value))) {
                $tr["<$name>"] = $this->encodeParams ? urlencode($value) : $value;
                unset($params[$name]);
